...
import { PlaywiseEngine, PlaylistSorter, SortCriteria, SortOrder, Song } from '@/lib/playwise-engine';
import { useToast } from '@/hooks/use-toast';

const Index = () => {
  const [engine] = useState(() => new PlaywiseEngine());
  const [searchQuery, setSearchQuery] = useState('');
  const [snapshot, setSnapshot] = useState(engine.exportSnapshot());
  const { toast } = useToast();

  // Update snapshot whenever data changes
  const updateSnapshot = () => {
    setSnapshot(engine.exportSnapshot());
  };
